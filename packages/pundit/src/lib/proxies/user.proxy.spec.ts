...
describe('UserProxy', () => {
  ...
  describe('get()', () => {
    it('gets user from hook', async () => {
      const userProxy = new UserProxy({ pundit: defaultPunditCache }, () => undefined);
  ...
  describe('getFromRequest()', () => {
    it('gets user from getUserFromRequest function', async () => {
      const userProxy = new UserProxy({ pundit: defaultPunditCache }, () => expectedUser);
  ...
  describe('getFromHook()', () => {
    it('gets cached user', async () => {
      const userProxy = new UserProxy({ pundit: { ...defaultPunditCache, user: expectedUser } }, () => undefined);
      expect(await userProxy.get()).toEqual(expectedUser);
    });
    ...
    it('gets user from hook', async () => {
      ...
      defaultPunditCache.hooks.user = new UserHook();
      const userProxy = new UserProxy({ pundit: defaultPunditCache }, () => requestUser);
